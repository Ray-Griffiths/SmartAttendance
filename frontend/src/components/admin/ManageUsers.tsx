...
import { Users, UserPlus, UserCheck, Edit2, Trash2, X, Loader2 } from "lucide-react";
...
const ManageUsers: React.FC = () => {
  ...
  const lecturersCount = users.filter((u) => (u.role || "Student") === "Lecturer").length;
  const studentsCount = users.filter((u) => (u.role || "Student") === "Student").length;
  const adminsCount = users.filter((u) => (u.role || "Student") === "Admin").length;
                    ...
                    {user.isEditing ? (
                      <div className="flex gap-2 items-center">
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            saveRole(user.id, e.target.value as User["role"])
                          }
                          className="border rounded px-2 py-1"
                          autoFocus
                        />
                      ...
                      <span className={roleColor(user.role || "Student")}>
                        {user.role || "Student"}
                      </span>
                    )}
                  </td>
